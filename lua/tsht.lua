...
function M.nodes()
	...
	local a = vim.api
	local p
	if a.nvim_buf_get_option(0, "filetype") == "javascriptreact" then
		p = get_parser(0, "javascript")
	elseif a.nvim_buf_get_option(0, "filetype") == "typescriptreact" then
		p = get_parser(0, "tsx")
	else
		p = get_parser(0)
	end
	local query = get_query(p._lang, "locals")
